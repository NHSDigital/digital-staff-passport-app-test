...
"""this class contains methods for the page
actions of review and confirm two creds"""
...
class ReviewAndConfirmTwoCredsPage(BasePage):
    ...
    def read_two_creds_home_page_heading(self):
        ...
        return self.read_value_from_element(
            self.two_creds_home_page_header_xpath,
            "two creds home page heading"
        )

    def click_two_creds_ready_review(self):
        """method to click two credentials ready to review"""
        return self.click_element(
            self.two_creds_home_page_2creds_ready_review_xpath,
            "click 2 creds review"
        )

    def read_creds_page_heading(self):
        """method to read the credentials page heading"""
        return self.read_value_from_element(
            self.creds_page_header_xpath,
            "credentials page heading"
        )

    def read_creds_page_creds_to_confirm_heading(self):
        """method to read the credentials page
        creds to confirm heading"""
        return self.read_value_from_element(
            self.creds_to_confirm_header_xpath,
            "creds to confirm heading"
        )

    def verify_prof_registration_cred(self):
        """method to verify if the prof registration
         cred is listed with action required"""
        return self.verify_element_displayed(
            self.prof_reg_cred_action_required_xpath,
            "prof reg cred action required"
        )

    def verify_employment_assign_cred(self):
        """method to verify if the employment assignment
         cred is listed with action required"""
        return self.verify_element_displayed(
    ...
    def read_creds_page_confirmed_creds_heading(self):
        """method to read the credentials
        page confirmed creds heading"""
        return self.read_value_from_element(
            self.confirmed_creds_header_xpath,
            "confirmed creds heading"
        )

    def verify_dbs_support_info_cred(self):
        """method to verify if the dbs info
        cred is listed in confirmed creds"""
        return self.verify_element_displayed(
            self.dbs_support_info_confirmed_cred_xpath,
            "dbs support info confirmed cred"
        )

    def verify_right_to_work_cred(self):
        """method to verify if the right to
         work cred is listed in confirmed creds"""
        return self.verify_element_displayed(
            self.right_to_work_confirmed_cred_xpath,
            "right to work confirmed cred"
        )

    def verify_identity_cred(self):
        """method to verify if the identity
        cred is listed in confirmed creds"""
        return self.verify_element_displayed(
            self.identity_confirmed_cred_xpath,
            "identity confirmed cred"
        )
    ...
    def read_emp_assignment_num_heading(self):
        """method to read the emp assign credential
        page assignment number heading"""
        return self.read_value_from_element(
            self.emp_assign_number_header_xpath,
            "emp assignment number heading"
        )

    def verify_emp_assign_cred_review_removed(self):
        """method to verify the employment assignment
        credential removed from review status"""
        return self.verify_element_displayed(
            self.employ_assign_acton_required_xpath,
            "emp assign creds to review"
        )

    def verify_emp_assign_cred_confirmed_listed(self):
        """method to verify the employment assignment
        credential listed with confirmed status"""
        return self.verify_element_displayed(
            self.employ_assign_confirmed_cred_xpath,
            "emp assign creds confirmed"
        )
